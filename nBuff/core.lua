...
local function SecondsToTimeAbbrevHook(seconds)
    ...
    if (seconds >= 86400) then
        ...
        return '|cffffffff%d|cffCC3333d|r', tempTime
    end

    if (seconds >= 3600) then
        tempTime = ceil(seconds / 3600)
        return '|cffffffff%d|cffCC3333h|r', tempTime
    end

    if (seconds >= 60) then
        tempTime = ceil(seconds / 60)
        return '|cffffffff%d|cffCC3333m|r', tempTime
    end
...
TempEnchant1:SetPoint('TOPRIGHT', Minimap, 'TOPLEFT', -55, 0)
...
for i = 1, NUM_TEMP_ENCHANT_FRAMES do
    ...
    button.Shadow:SetTexture('Interface\\AddOns\\pUI\\nBuff\\media\\textureShadow')
